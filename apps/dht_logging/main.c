...
#define DVI_TIMING dvi_timing_768x450p_60hz
...
void putline(uint Xstart, uint Ystart, uint Xend, uint Yend, uint rgb)
{
	int Xpoint = Xstart;
	int Ypoint = Ystart;
	// int dx, dy;
...
int main()
{
	...
	while (1)
	{
		...
		result.temp_celsius = read_onboard_temperature(TEMPERATURE_UNITS);
		result.humidity = 11;
		...
		for (int i = history_tail; i != history_head; i = (i + 1) % HISTORY_SIZE)
		{
			if(t==CHART_LEFT)
			{
				...
				putline(t-increment,temp_history[i-1] * temp_scale + temp_offset,t,temp_history[i] * temp_scale + temp_offset,RED);
			}
